export default class Adagrams {
  ...
  generateLetters = () => {
    ...
    // Add all the letters however many times they should appear to a new array
  ...
  drawLetters = () => {
    ...
    // Shuffle the letters
    letters = this.shuffleLetters(letters);
  ...
  usesAvailableLetters = (input, lettersInHand) => {
    ...
    const frequencyWord = this.countLetterFrequency(input);
    const frequencyHand = this.countLetterFrequency(lettersInHand);

    // Compare the word frequency in the input and in lettersInHand
  ...
  scoreWord = (word) => {
    ...
    // Add up the points by looking up each letter in word in the SCORE_CHART object
  ...
  highestScoreFrom = (words) => {
    ...
    for (const word of words) {
      ...
      if (!(word in wordMap)) {
        wordMap[word] = [word.length, this.scoreWord(word)]
      }
    }
