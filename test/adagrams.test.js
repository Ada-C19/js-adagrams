...
describe("Adagrams", () => {
  describe("drawLetters", () => {
    it("draws ten letters from the letter pool", () => {
      const player = new Adagrams();
      const drawn = player.drawLetters();
  ...
  describe("usesAvailableLetters", () => {
    it("returns true if the submitted letters are valid against the drawn letters", () => {
      ...
      const isValid = player.usesAvailableLetters(word, drawn);
  ...
  describe("scoreWord", () => {
    ...
    const expectScores = (wordScores) => {
      Object.entries(wordScores).forEach(([word, score]) => {
        expect(player.scoreWord(word)).toBe(score);
      });
    };
  ...
  describe("highestScoreFrom", () => {
    it("returns a hash that contains the word and score of best word in an array", () => {
      ...
      const correct = { word: "XXXX", score: player.scoreWord("XXXX") };

      expect(player.highestScoreFrom(words)).toEqual(correct);
    });
    ...
    describe("in case of tied score", () => {
      ...
      const expectTie = (words) => {
        const scores = words.map((word) => player.scoreWord(word));
      ...
      it("selects the word with 10 letters", () => {
        ...
        const correct = {
          word: "AAAAAAAAAA",
          score: player.scoreWord("AAAAAAAAAA"),
        };
        expectTie(words);

        expect(player.highestScoreFrom(words)).toEqual(correct);
        expect(player.highestScoreFrom(words.reverse())).toEqual(correct);
      });

      it("selects the word with fewer letters when neither are 10 letters", () => {
        const player = new Adagrams();
        const words = ["MMMM", "WWW"];
        const correct = { word: "WWW", score: player.scoreWord("WWW") };
        expectTie(words);
      ...
      it("selects the first word when both have same length", () => {
        ...
        const second = {
          word: "EEEEEEEEEE",
          score: player.scoreWord("EEEEEEEEEE"),
        };
        expectTie(words);

        expect(player.highestScoreFrom(words)).toEqual(first);
        expect(player.highestScoreFrom(words.reverse())).toEqual(second);
      });
    });
  });
});
